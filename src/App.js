...
import { Plus, Minus, Info, Trash2, Search, Sparkles, Store, Truck, Droplet, Scissors, Shirt, Home, Wind, ShoppingBag, Menu } from 'lucide-react';
...
export default function FlyDryEstimator() {
  ...
  // Mobile Tab State
  const [mobileTab, setMobileTab] = useState('menu'); // 'menu' | 'basket'
  ...
  return (
    <div className="w-full max-w-6xl mx-auto bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden flex flex-col font-sans text-gray-800 h-[800px]">

      {/* --- MOBILE TAB NAVIGATION (Hidden on Desktop) --- */}
      <div className="flex md:hidden bg-[#114232] p-2 gap-2 shrink-0 z-20 shadow-md">
        <button
          onClick={() => setMobileTab('menu')}
          className={`flex-1 py-3 text-[13px] font-bold rounded-xl transition-all flex items-center justify-center gap-2 ${
            mobileTab === 'menu' ? 'bg-white text-[#114232] shadow-sm' : 'text-[#C5A059] hover:bg-[#0a2b20]'
          }`}
        >
          <Menu size={16} />
          Service Menu
        ...
        <button
          ...
          className={`flex-1 py-3 text-[13px] font-bold rounded-xl transition-all flex items-center justify-center gap-2 ${
            mobileTab === 'basket' ? 'bg-white text-[#114232] shadow-sm' : 'text-[#C5A059] hover:bg-[#0a2b20]'
          }`}
        >
          <ShoppingBag size={16} />
          Your Basket
          {cart.length > 0 && (
            <span className={`px-2 py-0.5 rounded-full text-[11px] ${
              mobileTab === 'basket' ? 'bg-[#114232] text-white' : 'bg-[#C5A059] text-[#114232]'
            }`}>
      ...
      <div className="flex-1 flex flex-row overflow-hidden w-full">
        ...
        <div className={`flex-1 flex-col bg-gray-50 md:border-r border-gray-200 ${mobileTab === 'menu' ? 'flex' : 'hidden md:flex'}`}>
        ...
        <div className={`w-full md:w-[420px] bg-gray-50/50 flex-col md:border-l border-gray-200 ${mobileTab === 'basket' ? 'flex' : 'hidden md:flex'}`}>
